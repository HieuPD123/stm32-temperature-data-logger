...
* **Cú pháp:** `READ page_number`
* **Tham số:** `page_number` là số nguyên (bắt đầu từ `0`).
    * `READ 0`: Đọc từ dòng 0 đến dòng 9 (10 dòng đầu tiên của file).
    * `READ 1`: Đọc từ dòng 10 đến dòng 19.
    * `READ 2`: Đọc từ dòng 20 đến dòng 29.
* **Cơ chế hoạt động:** Để tiết kiệm RAM và tài nguyên MCU, hệ thống sẽ lướt nhanh qua các dòng trước đó và chỉ gọi hàm truyền UART cho đúng 10 dòng thuộc trang được yêu cầu, sau đó đóng file sớm.
...
Dùng để cài đặt lại thời gian cho chip RTC nội của STM32 khi hệ thống mới khởi động hoặc khi bị sai lệch thời gian. Thời gian này sẽ trực tiếp áp dụng vào nội dung file log.

Cú pháp: RTC:YYYY-MM-DD HH:MM:SS

Ý nghĩa các tham số:

YYYY: Năm (4 chữ số, ví dụ: 2026)
...
Lưu ý quan trọng: Giữa từ khóa RTC: và năm YYYY không được có khoảng trắng.

Ví dụ tương tác:

Plaintext
Gửi:    RTC:2026-06-17 11:15:00
Phản hồi từ STM32:
System RTC Updated to: 2026-06-17 11:15:00
🛠 Định Dạng File Log Trên Thẻ Nhớ
Dữ liệu nhiệt độ được lưu trữ trong file tĩnh log.csv theo định dạng chuẩn CSV, phân tách bằng dấu phẩy , giúp dễ dàng import thẳng vào Excel hoặc các công cụ phân tích để vẽ đồ thị:
